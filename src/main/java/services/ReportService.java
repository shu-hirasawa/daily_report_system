...
public class ReportService extends ServiceBase {
    ...
    /**
     * nameを条件に取得したデータをReportViewのインスタンスで返却する
     * @param name
     * @return 取得データのインスタンス
     */
    public ReportView findName(String name) {
        return ReportConverter.toView(findNameInternal(name));
    }
     ...
     * @return 取得データのインスタンス
     */
    private Report findNameInternal(String name) {
        return em.find(Report.class, name);
    }

    /**
     * keywordに該当した従業員が作成した日報データを取得しReportViewのリストで返却する
     * @param employee 従業員
     * @param page ページ数
     * @return 一覧画面に表示するデータのリスト
     */

    public List<ReportView> getSerchPerPage(String keyword, int page) {

        List<Report> reportsSerch = em.createNamedQuery(JpaConst.Q_REP_GET_SERCH_MINE, Report.class)
                .setParameter("keyword", "%" + keyword + "%")
                .setFirstResult(JpaConst.ROW_PER_PAGE * (page - 1))
                .setMaxResults(JpaConst.ROW_PER_PAGE)
                .getResultList();
        return ReportConverter.toViewList(reportsSerch);
    }

    /**
     * keywordに該当した従業員が作成した日報データの件数を取得し、返却する
     * @param employee
     * @return 日報データの件数
     */
    public long countSerchMine(String keyword) {

        long reports_count = (long) em.createNamedQuery(JpaConst.Q_REP_COUNT_SERCH_MINE, Long.class)
                .setParameter("keyword", "%" + keyword + "%")
                .getSingleResult();
